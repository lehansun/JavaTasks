...
public class Snake {
    ...
    public void move(Apple apple) {
        ...
        if (isAbroad(newHead) || checkCollision(newHead)) {
            isAlive = false;
        } else {
    ...
    public void setDirection(Direction direction) {
        ...
        if (direction == Direction.LEFT && snakeParts.get(0).y == snakeParts.get(1).y) return;
        if (direction == Direction.RIGHT && snakeParts.get(0).y == snakeParts.get(1).y) return;
        if (direction == Direction.UP && snakeParts.get(0).x == snakeParts.get(1).x) return;
        if (direction == Direction.DOWN && snakeParts.get(0).x == snakeParts.get(1).x) return;
    ...
    private boolean isAbroad(GameObject newHead) {
        return newHead.x >= SnakeGame.WIDTH
                || newHead.x < 0
